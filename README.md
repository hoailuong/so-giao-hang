...
Sổ Giao Hàng là một ứng dụng web (PWA) được thiết kế tối ưu cho thiết bị di động, giúp quản lý, theo dõi đơn hàng, đối chiếu công nợ và tiến độ doanh số hàng tháng. Phiên bản v8.0 tích hợp sức mạnh của Trí tuệ nhân tạo (Google Gemini AI) để tự động hóa việc nhập liệu và hệ thống Đồng bộ Đám mây (Firebase) theo thời gian thực.
...
2. 🤖 Trợ Lý AI Nhập Liệu & Đối Chiếu (Google Gemini)
...
Thì ra code file index.html ở Github sửa lại là:
...
2. Mở khóa Database: Vào Firestore Database > Tạo Database (Test mode). Chuyển sang tab Rules và sửa quy tắc thành:
javascript
rules_version = '2';
...
5. Dán API Key vào và bấm Lưu Cài Đặt. (API Key sẽ được lưu an toàn tuyệt đối ngay trên trình duyệt của bạn).
